...
public class PricingService {
  ...
  private void process(List<EmoticonRate> newPrices) {
    for (EmoticonRate emoticonRate : newPrices) {
      ...
      if (Math.abs(emoticonPrice.getPrice() - emoticonRate.getPerMinute()) > 2) {
        emoticonPrice.setPrice(emoticonRate.getPerMinute());
        if(emoticonPrice.getPrice() > 10){
  ...
  public void channelMessage(String json) {
    ...
    if (changes) {
      ...
      for (ChannelStats channelStats : newStats) {
        ChannelRecord channelRecord = channelRecordRepository.findByChannelNameAndEmoteName(channelStats.getChannel(),
            channelStats.getTopEmote());
